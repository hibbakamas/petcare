...
from app.models import db, Users, Household, Pet, Entry
...
def _hh_pet(client, app, user="entries_owner"):
    ...
    hh_post = find_api_route(app, ["household"], "POST") or find_api_route(app, ["households"], "POST")
    hid = extract_int(client.post(hh_post, json={"name":"EHH"}).get_json(silent=True) or {}, ("id","household_id"))
    ...
    pid = extract_int(client.post(purl, json={"name":"EPet","household_id":hid}).get_json(silent=True) or {}, ("id","pet_id"))
    return hid, pid

# ---------- tests ----------

def test_create_and_optionally_update_delete_entry(client, app):
    _login(client, app)  # uses default username
    ...
    if entries_put and eid:
        ...
        # include 405 if your API doesn't support update
...
def test_entries_validation_create_update_delete_and_ownership(client, app):
    ...
    # different user should not be able to update/delete
    client.get("/logout")
...
# --- tiny helpers (kept local so we don't touch your fixtures) ---
...
def test_entries_create_401_when_not_logged_in(client, app):
    ...
    assert r.status_code == 401  # @login_required_api
...
def test_entries_create_404_when_pet_missing(client, app):
    ...
    assert r.status_code == 404  # Pet.query.get_or_404

def test_entries_create_400_when_content_missing_or_blank(client, app):
    uid = _mk_user(app); _login_as(client, uid)
    hid = _mk_household(app); pid = _mk_pet(app, hid)
