"""Users API: optional 'update current user' smoke test."""
...
def test_update_current_user_username_if_supported(client, app):
    _login(client, app)
    # Locate a plausible PATCH endpoint for the current user.
    patch_me = (
        find_api_route(app, ["users", "me"], "PATCH")
        or find_api_route(app, ["users", "current"], "PATCH")
        or find_api_route(app, ["users"], "PATCH")
    )
    if not patch_me:
